...
- **⚠️ Cache-Busting in `app/`:** Alle `css/`- und `js/`-Einbindungen in den `app/*.html` haben `?v=N` (aktuell **`?v=19`**). **Bei jeder Änderung an app/ CSS/JS die Nummer hochzählen**, sonst greift der Cache weiter: `perl -pi -e 's/\?v=19"/?v=20"/g' app/*.html`. (Wirkt erst, wenn der Browser die neue HTML geladen hat — beim ersten Mal trotzdem privates Tab.)
...
- **Erika auf der Startseite zunächst eingeklappt (neuer „?"-Button, NICHT zu verwechseln mit dem alten, entfernten Tutorial-„?"-Button aus Zeile oben):** Auf `index.html` (beide Modi) erscheint statt der großen Figur zunächst nur ein „?"-Button unten rechts. Umgesetzt als neuer Zustand `.erika.collapsed` in `erika.js`/`erika.css` (`Erika.startCollapsed()`), **nur in `index.html` aufgerufen** — auf allen anderen Seiten (Einstellungen, Über/Datenschutz, Stufenauswahl der Übungen) bleibt Erika unverändert direkt sichtbar (`.erika-help-btn` dort `display:none`).
  - **„?"-Button quadratisch statt rund, gleiche Größe wie der Einstellungen-Button:** `.erika-help-btn` und `.profile-btn` (Einstellungen, nur `index.html`) sind beide **4.5rem × 4.5rem, `border-radius:14px`** (Radius = häufigster Button-Wert im Projekt, siehe `.setting-row`/`.sensor-btn`). `.profile-btn` zeigt jetzt nur noch das Zahnrad-Emoji (Text „Einstellungen" entfernt, `aria-label="Einstellungen"` für Screenreader). Geprüft: beide exakt 72×72px, radius 14px.
