...
- **⚠️ Cache-Busting in `app/`:** Alle `css/`- und `js/`-Einbindungen in den `app/*.html` haben `?v=N` (aktuell **`?v=17`**). **Bei jeder Änderung an app/ CSS/JS die Nummer hochzählen**, sonst greift der Cache weiter: `perl -pi -e 's/\?v=17"/?v=18"/g' app/*.html`. (Wirkt erst, wenn der Browser die neue HTML geladen hat — beim ersten Mal trotzdem privates Tab.)
...
- **Erika auf der Startseite zunächst eingeklappt (neuer „?"-Button, NICHT zu verwechseln mit dem alten, entfernten Tutorial-„?"-Button aus Zeile oben):** Auf `index.html` (beide Modi) erscheint statt der großen Figur zunächst nur ein runder „?"-Button unten rechts. Umgesetzt als neuer Zustand `.erika.collapsed` in `erika.js`/`erika.css` (`Erika.startCollapsed()`), **nur in `index.html` aufgerufen** — auf allen anderen Seiten (Einstellungen, Übungen, Über/Datenschutz) bleibt Erika unverändert direkt sichtbar (`.erika-help-btn` dort `display:none`).
  ...
  - **Die Figur (`.erika-info-fig`) bleibt an ihrem gewohnten Platz unten rechts** (`position:fixed; right:clamp(10px,3vw,30px); bottom:max(16px,env(safe-area-inset-bottom))` — exakt wie `.erika` sonst), NICHT mittig über dem Textfeld. Aus dem Flex-Fluss von `.erika-info` gelöst; nur Textfeld + Button folgen der 40 %-Positionierung. Etwas größer als die normale Figur (`clamp(130px, min(34vh,42vw), 400px)` vs. sonst `clamp(110px,...,360px)`).
