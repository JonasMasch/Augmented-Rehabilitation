...
- **⚠️ Cache-Busting in `app/`:** Alle `css/`- und `js/`-Einbindungen in den `app/*.html` haben `?v=N` (aktuell **`?v=9`**). **Bei jeder Änderung an app/ CSS/JS die Nummer hochzählen**, sonst greift der Cache weiter: `perl -pi -e 's/\?v=9"/?v=10"/g' app/*.html`. (Wirkt erst, wenn der Browser die neue HTML geladen hat — beim ersten Mal trotzdem privates Tab.)
...
- **⭐ Neglect-Layout — linke 40 % frei (NUR im Einfach-Modus):** Die linke Zone (40 %) bleibt frei; alle Navigations-/Bedienelemente sitzen **linksbündig direkt an der 40 %-Kante** (so weit links wie erlaubt — zieht die Aufmerksamkeit möglichst weit nach links). Dort erscheinen sonst nur die **Übungsobjekte**. **Ausgenommen:** `.center-zone` (Ziel/Kreis) und die per-JS über `innerWidth` positionierten Objekte (Käfer/Schnecke/Salate) — die dürfen weiter nach links.
  - Zentraler Block am Ende von `common.css`: CSS-Var `--free-left` (`40vw` im Einfach-Modus, sonst `0`). Aktiv über `html[data-mode="patient"]` (**nur Startseite**) **ODER** `html.flow-mode` (geführte Übung, `?flow=n`).
  - **Die EINSTELLUNGEN bleiben in BEIDEN Modi normal mittig** (kein Shift — bewusst so gewollt). **Erweitert-Modus** (`data-mode="pflege"` bzw. Standalone-Übung ohne `flow-mode`) bleibt unverändert (`--free-left:0`).
  - Wirkung: Startseiten-Spalte `.home` linksbündig via `padding-left:var` + `align-items:flex-start`; der innere Wrapper hat dafür die Klasse **`.home-col`** (in `index.html`; `.patient-only` ebenfalls auf `flex-start`, sonst zentriert der den Start-Button erneut). Mittige Übungs-Elemente (`.instr`, `.seq-list`) über `left:var; transform:none`; `.audio-bars/-label` eingerückt; Vollflächen-Overlays (`.success`, `.erika-pause` → `align-items:flex-start`; `.intro-overlay` → `justify-content:flex-start`) behalten den Backdrop, Inhalt linksbündig an der Kante. Geprüft (Querformat 1024×640): alle Bedienelemente exakt bei L=410 px (=40vw), Einstellungen mittig, Erweitert-Modus unberührt.
